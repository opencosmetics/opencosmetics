...
public class RightArmRenderer extends RendererBase {


    public RightArmRenderer(ResourceLocation texture, ModelBase model) {
        super(texture, model);
    }
    ...
    public void render(RenderPlayerEvent.Pre event, EntityPlayer player) {
        ...
        ModelRenderer rightArm = event.renderer.getMainModel().bipedRightArm;

        GlStateManager.rotate((float)((rightArm.rotateAngleX * 180D) / Math.PI), 1.0F, 0.0F, 0.0F);
        GlStateManager.rotate((float)((rightArm.rotateAngleZ * 180D) / Math.PI), 0.0F, 0.0F, 1.0F);
