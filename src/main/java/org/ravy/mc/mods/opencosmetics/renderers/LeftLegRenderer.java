...
public class LeftLegRenderer extends RendererBase {


    public LeftLegRenderer(ResourceLocation texture, ModelBase model) {
        super(texture, model);
    }
    ...
    public void render(RenderPlayerEvent.Pre event, EntityPlayer player) {
        ...
        ModelRenderer leftLeg = event.renderer.getMainModel().bipedLeftLeg;

        GlStateManager.rotate((float)((leftLeg.rotateAngleX * 180D) / Math.PI), 1.0F, 0.0F, 0.0F);
        GlStateManager.rotate((float)((leftLeg.rotateAngleZ * 180D) / Math.PI), 0.0F, 0.0F, 1.0F);
